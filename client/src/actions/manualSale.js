...
export const publishOrders = (id, value) => async (dispatch) => {
  ...
  try {
    const res = await axios.put(
      `/api/notification_manual_sales/publish_status/${id}`,
      value,
      config
    );
    dispatch({
...
//   try {
//     const res = await axios.get("/api/data_product");
...
export const updateProduct = (id, formData) => async (dispatch) => {
  ...
  try {
    const res = await axios.put(
      `/api/notification_manual_sales/${id}`,
      formData,
      config
    );
    dispatch({
