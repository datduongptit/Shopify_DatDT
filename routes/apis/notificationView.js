...
router.post('/', async(req, res) => {
    ...
    try {
        conn.query('SELECT * FROM notification_view', (error, results, fields) => {
            ...
            if(results.length === 0) {
                conn.query("INSERT INTO notification_view SET ?", notiFields , async function (error, results, fields) {
                    if (error) throw error;
                    return res.send({ error: false, data: results, message: 'Created' });
                });
            };
            conn.query("UPDATE notification_view SET ? WHERE 1", notiFields , async function (error, results, fields) {
                if (error) throw error;
